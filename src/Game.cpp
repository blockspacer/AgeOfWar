...
Game::Game(const char* save) : m_save(save), m_turnNo(1), m_bluePlayer(*new Player(Player::Side::blue)),
                               m_redPlayer(*new Player(Player::Side::red)),
                               m_playground(m_bluePlayer.base(), m_redPlayer.base()) {
    std::string s = save;
    s += + ".sav";
...
void Game::turn() {
	...
	std::cout << *this << std::endl << ansi::color("waiting blue playing...", ansi::normal_blue_foreground) << std::endl;
	...
	std::cout << *this << std::endl << ansi::color("waiting red playing...", ansi::normal_red_foreground) << std::endl;
...
std::ostream &operator<<(std::ostream &os, const Game &game) {
    ...
    /* GOLD */
    std::string blueGold = std::to_string(game.bluePlayer().gold()) + 'g', redGold = std::to_string(game.redPlayer().gold()) + 'g';
